...
export default class EmailVerificationToken extends BaseModel {
  ...
  static generateToken(): { plainToken: string; hashedToken: string } {
    ...
    const hashedToken = EmailVerificationToken.hashToken(plainToken)
   ...
   */
  static async findByPlainToken(plainToken: string): Promise<EmailVerificationToken | null> {
    const hashedToken = EmailVerificationToken.hashToken(plainToken)
    return EmailVerificationToken.query().where('token', hashedToken).preload('user').first()
  }
  ...
  static async deleteByPlainToken(plainToken: string): Promise<void> {
    const hashedToken = EmailVerificationToken.hashToken(plainToken)
    await EmailVerificationToken.query().where('token', hashedToken).delete()
  }
}
